...
import { Plus, Link2, LayoutGrid, List, Loader2, FolderOpen } from 'lucide-react'
...
export default function CommonLinks({ variant = 'personal' }: Props) {
  const navigate = useNavigate()
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid')
  const [modalOpen, setModalOpen] = useState(false)
  const [addLinkOpen, setAddLinkOpen] = useState(false)
  const { data: rawCategories, isLoading } = useMyCategories('professional')
  ...
  const categoryPath = (id: string) =>
    variant === 'professional' ? `/professional-categories/${id}` : `/categories/${id}`
          ...
          >
            {categories.map((cat) => {
              ...
              return (
                <div
                  key={cat._id}
                  onClick={() => navigate(categoryPath(cat._id))}
  ...
  return (
    <>
      ...
      <CreateCategoryModal open={modalOpen} onClose={() => setModalOpen(false)} context="professional" />
      <AddLinkModal open={addLinkOpen} onClose={() => setAddLinkOpen(false)} context="professional" />
    </>
  )
}
