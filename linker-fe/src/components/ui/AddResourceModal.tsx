...
export default function AddResourceModal({ open, onClose, onSubmit, isPending }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const valid = resources.filter((r) => r.title.trim() && r.url.trim())
  ...
  const hasValid = resources.some((r) => r.title.trim() && r.url.trim())
    ...
    >
      <div className="w-full max-w-md bg-surface border border-border rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div>
            <h2 className="text-sm font-bold text-foreground">Add Resources</h2>
            <p className="text-xs text-muted-foreground mt-0.5">Save links and documents for the team.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
                ...
                <div key={idx} className="flex items-start gap-2">
                  ...
                  {resources.length > 1 && (
                    <button
                      ...
                      className="p-2 mt-0.5 border border-border text-muted-foreground rounded-xl hover:bg-danger/10 hover:text-danger transition-colors shrink-0 cursor-pointer"
